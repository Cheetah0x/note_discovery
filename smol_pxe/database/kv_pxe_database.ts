...
export class KVPxeDatabase implements PxeDatabase {
  ...
  constructor(private db: AztecKVStore) {
    ...
    this.#nullifiersByBlockNumber = db.openMultiMap(
      "nullifier_to_block_number"
    );

    this.#nullifiedNotesToScope = db.openMultiMap("nullified_notes_to_scope");
    this.#nullifiedNotesByContract = db.openMultiMap(
      "nullified_notes_by_contract"
    );
    this.#nullifiedNotesByStorageSlot = db.openMultiMap(
      "nullified_notes_by_storage_slot"
    );
    this.#nullifiedNotesByTxHash = db.openMultiMap(
      "nullified_notes_by_tx_hash"
    );
    this.#nullifiedNotesByAddressPoint = db.openMultiMap(
      "nullified_notes_by_address_point"
    );
    this.#nullifiedNotesByNullifier = db.openMap(
      "nullified_notes_by_nullifier"
    );

    this.#outgoingNotes = db.openMap("outgoing_notes");
    this.#outgoingNotesByContract = db.openMultiMap(
      "outgoing_notes_by_contract"
    );
    this.#outgoingNotesByStorageSlot = db.openMultiMap(
      "outgoing_notes_by_storage_slot"
    );
    ...
    for (const scope of this.#scopes.entries()) {
      this.#notesByContractAndScope.set(
        scope,
        db.openMultiMap(`${scope}:notes_by_contract`)
      );
      this.#notesByStorageSlotAndScope.set(
        scope,
        db.openMultiMap(`${scope}:notes_by_storage_slot`)
      );
      this.#notesByTxHashAndScope.set(
        scope,
        db.openMultiMap(`${scope}:notes_by_tx_hash`)
      );
      this.#notesByAddressPointAndScope.set(
        scope,
        db.openMultiMap(`${scope}:notes_by_address_point`)
      );
    }

    this.#taggingSecretIndexesForSenders = db.openMap(
      "tagging_secret_indexes_for_senders"
    );
    this.#taggingSecretIndexesForRecipients = db.openMap(
      "tagging_secret_indexes_for_recipients"
    );
  }
  ...
  public async getContract(
    ...
    const artifact =
      instance && (await this.getContractArtifact(instance?.contractClassId));
  ...
  public async addContractArtifact(
    ...
    const privateSelectors = contract.functions
      .filter(
        (functionArtifact) =>
          functionArtifact.functionType === FunctionType.PRIVATE
      )
    ...
    await this.#contractArtifacts.set(
      id.toString(),
      contractArtifactToBuffer(contract)
    );
  }
  ...
  public getContractArtifact(id: Fr): Promise<ContractArtifact | undefined> {
    ...
    return Promise.resolve(
      contract && contractArtifactFromBuffer(Buffer.from(contract))
    );
  }
  ...
  getContractInstance(
    ...
    return Promise.resolve(
      contract &&
        SerializableContractInstance.fromBuffer(contract).withAddress(address)
    );
  }

  getContractsAddresses(): Promise<AztecAddress[]> {
    return Promise.resolve(
      Array.from(this.#contractInstances.keys()).map(AztecAddress.fromString)
    );
  }
  ...
  async addNotes(
    ...
    return this.db.transaction(() => {
      for (const dao of incomingNotes) {
        ...
        void this.#nullifierToNoteId.set(
          dao.siloedNullifier.toString(),
          noteIndex
        );

        void this.#notesByContractAndScope
          .get(scope.toString())!
          .set(dao.contractAddress.toString(), noteIndex);
        void this.#notesByStorageSlotAndScope
          .get(scope.toString())!
          .set(dao.storageSlot.toString(), noteIndex);
        void this.#notesByTxHashAndScope
          .get(scope.toString())!
          .set(dao.txHash.toString(), noteIndex);
        void this.#notesByAddressPointAndScope
          .get(scope.toString())!
          .set(dao.addressPoint.toString(), noteIndex);
      }

      for (const dao of outgoingNotes) {
        const noteIndex = toBufferBE(dao.index, 32).toString("hex");
        void this.#outgoingNotes.set(noteIndex, dao.toBuffer());
        void this.#outgoingNotesByContract.set(
          dao.contractAddress.toString(),
          noteIndex
        );
        void this.#outgoingNotesByStorageSlot.set(
          dao.storageSlot.toString(),
          noteIndex
        );
    ...
    return this.db.transaction(() => {
      for (const note of this.#notes.values()) {
        ...
        if (noteDao.l2BlockNumber > blockNumber) {
          ...
          void this.#nullifierToNoteId.delete(
            noteDao.siloedNullifier.toString()
          );
          for (const scope of this.#scopes.entries()) {
            void this.#notesByAddressPointAndScope
              .get(scope)!
              .deleteValue(noteDao.addressPoint.toString(), noteIndex);
            void this.#notesByTxHashAndScope
              .get(scope)!
              .deleteValue(noteDao.txHash.toString(), noteIndex);
            void this.#notesByContractAndScope
              .get(scope)!
              .deleteValue(noteDao.contractAddress.toString(), noteIndex);
            void this.#notesByStorageSlotAndScope
              .get(scope)!
              .deleteValue(noteDao.storageSlot.toString(), noteIndex);
          }
        }
      }
      ...
      for (const note of this.#outgoingNotes.values()) {
        ...
        if (noteDao.l2BlockNumber > blockNumber) {
          ...
          void this.#outgoingNotesByContract.deleteValue(
            noteDao.contractAddress.toString(),
            noteIndex
          );
          void this.#outgoingNotesByStorageSlot.deleteValue(
            noteDao.storageSlot.toString(),
            noteIndex
          );
          void this.#outgoingNotesByTxHash.deleteValue(
            noteDao.txHash.toString(),
            noteIndex
          );
          void this.#outgoingNotesByOvpkM.deleteValue(
            noteDao.ovpkM.toString(),
            noteIndex
          );
        }
      }
  ...
  public async unnullifyNotesAfter(blockNumber: number): Promise<void> {
    ...
    const notesIndexesToReinsert = await this.db.transaction(() =>
      nullifiersToUndo.map((nullifier) =>
        this.#nullifiedNotesByNullifier.get(nullifier)
      )
    );
    ...
    await this.db.transaction(() => {
      for (const dao of noteDaos) {
        ...
        let scopes = Array.from(
          this.#nullifiedNotesToScope.getValues(noteIndex) ?? []
        );
        ...
        void this.#nullifiersByBlockNumber.deleteValue(
          dao.l2BlockNumber,
          dao.siloedNullifier.toString()
        );
        void this.#nullifiedNotesByContract.deleteValue(
          dao.contractAddress.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByStorageSlot.deleteValue(
          dao.storageSlot.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByTxHash.deleteValue(
          dao.txHash.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByAddressPoint.deleteValue(
          dao.addressPoint.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByNullifier.delete(
          dao.siloedNullifier.toString()
        );
      }
    });
  ...
  getIncomingNotes(filter: IncomingNotesFilter): Promise<IncomingNoteDao[]> {
    ...
    filter.scopes ??= [...this.#scopes.entries()].map((addressString) =>
    ...
    for (const scope of new Set(filter.scopes)) {
      ...
      if (!this.#scopes.has(formattedScopeString)) {
        throw new Error(
          "Trying to get incoming notes of an scope that is not in the PXE database"
        );
      }

      activeNoteIdsPerScope.push(
        publicKey
          ? this.#notesByAddressPointAndScope
              .get(formattedScopeString)!
              .getValues(publicKey.toString())
          : filter.txHash
          ? this.#notesByTxHashAndScope
              .get(formattedScopeString)!
              .getValues(filter.txHash.toString())
          : filter.contractAddress
          ? this.#notesByContractAndScope
              .get(formattedScopeString)!
              .getValues(filter.contractAddress.toString())
          : filter.storageSlot
          ? this.#notesByStorageSlotAndScope
              .get(formattedScopeString)!
              .getValues(filter.storageSlot.toString())
          : this.#notesByAddressPointAndScope
              .get(formattedScopeString)!
              .values()
      );
    ...
    candidateNoteSources.push({
      ids: new Set(
        activeNoteIdsPerScope.flatMap((iterableIterator) => [
          ...iterableIterator,
        ])
      ),
      notes: this.#notes,
    ...
    if (filter.status == NoteStatus.ACTIVE_OR_NULLIFIED) {
      candidateNoteSources.push({
        ids: publicKey
          ...
          : filter.contractAddress
          ? this.#nullifiedNotesByContract.getValues(
              filter.contractAddress.toString()
            )
          : filter.storageSlot
          ? this.#nullifiedNotesByStorageSlot.getValues(
              filter.storageSlot.toString()
            )
    ...
    for (const { ids, notes } of candidateNoteSources) {
      for (const id of ids) {
        ...
        if (
          filter.contractAddress &&
          !note.contractAddress.equals(filter.contractAddress)
        ) {
          continue;
        }
        ...
        if (
          filter.storageSlot &&
          !note.storageSlot.equals(filter.storageSlot!)
        ) {
          continue;
        }
        ...
        if (
          filter.siloedNullifier &&
          !note.siloedNullifier.equals(filter.siloedNullifier)
        ) {
          continue;
        }
  ...
  getOutgoingNotes(filter: OutgoingNotesFilter): Promise<OutgoingNoteDao[]> {
    const ovpkM: PublicKey | undefined = filter.owner
      ? this.#getCompleteAddress(filter.owner)?.publicKeys
  ...
  removeNullifiedNotes(
    nullifiers: InBlock<Fr>[],
    accountAddressPoint: PublicKey
    ...
    return this.#db.transaction(() => {
      ...
      for (const blockScopedNullifier of nullifiers) {
        const { data: nullifier, l2BlockNumber: blockNumber } =
          blockScopedNullifier;
        ...
        for (const scope of this.#scopes.entries()) {
          void this.#notesByAddressPointAndScope
            .get(scope)!
            .deleteValue(accountAddressPoint.toString(), noteIndex);
          void this.#notesByTxHashAndScope
            .get(scope)!
            .deleteValue(note.txHash.toString(), noteIndex);
          void this.#notesByContractAndScope
            .get(scope)!
            .deleteValue(note.contractAddress.toString(), noteIndex);
          void this.#notesByStorageSlotAndScope
            .get(scope)!
            .deleteValue(note.storageSlot.toString(), noteIndex);
        }
        ...
        void this.#nullifiersByBlockNumber.set(
          blockNumber,
          nullifier.toString()
        );
        void this.#nullifiedNotesByContract.set(
          note.contractAddress.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByStorageSlot.set(
          note.storageSlot.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByTxHash.set(
          note.txHash.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByAddressPoint.set(
          note.addressPoint.toString(),
          noteIndex
        );
        void this.#nullifiedNotesByNullifier.set(
          nullifier.toString(),
          noteIndex
        );
  ...
  async addNullifiedNote(note: IncomingNoteDao): Promise<void> {
    ...
    await this.#nullifiedNotesByContract.set(
      note.contractAddress.toString(),
      noteIndex
    );
    await this.#nullifiedNotesByStorageSlot.set(
      note.storageSlot.toString(),
      noteIndex
    );
    await this.#nullifiedNotesByTxHash.set(note.txHash.toString(), noteIndex);
    await this.#nullifiedNotesByAddressPoint.set(
      note.addressPoint.toString(),
      noteIndex
    );
  ...
  getBlockNumber(): number | undefined {
    ...
    return Number(
      Header.fromBuffer(headerBuffer).globalVariables.blockNumber.toBigInt()
    );
  }
  ...
  async #addScope(scope: AztecAddress): Promise<boolean> {
    ...
    this.#notesByContractAndScope.set(
      scopeString,
      this.#db.openMultiMap(`${scopeString}:notes_by_contract`)
    );
    this.#notesByStorageSlotAndScope.set(
      scopeString,
    ...
    this.#notesByTxHashAndScope.set(
      scopeString,
      this.#db.openMultiMap(`${scopeString}:notes_by_tx_hash`)
    );
    this.#notesByAddressPointAndScope.set(
      scopeString,
  ...
  ): Promise<CompleteAddress | undefined> {
    return Promise.resolve(this.#getCompleteAddress(account));
  }

  getCompleteAddresses(): Promise<CompleteAddress[]> {
    return Promise.resolve(
      Array.from(this.#completeAddresses).map((v) =>
        CompleteAddress.fromBuffer(v)
      )
    );
  }
  ...
  setSynchedBlockNumberForAccount(
    account: AztecAddress,
    blockNumber: number
  ): Promise<void> {
  ...
  async estimateSize(): Promise<number> {
    const incomingNotesSize = Array.from(
      await this.getIncomingNotes({})
    ).reduce((sum, note) => sum + note.getSize(), 0);
    const outgoingNotesSize = Array.from(
      await this.getOutgoingNotes({})
    ).reduce((sum, note) => sum + note.getSize(), 0);
    ...
    const addressesSize =
      this.#completeAddresses.length * CompleteAddress.SIZE_IN_BYTES;
  ...
  async setTaggingSecretsIndexesAsSender(
    indexedSecrets: IndexedTaggingSecret[]
  ): Promise<void> {
    await this.#setTaggingSecretsIndexes(
      indexedSecrets,
      this.#taggingSecretIndexesForSenders
    );
  }
  ...
  async setTaggingSecretsIndexesAsRecipient(
    ...
    await this.#setTaggingSecretsIndexes(
      indexedSecrets,
      this.#taggingSecretIndexesForRecipients
    );
  }
  ...
  async getTaggingSecretsIndexesAsRecipient(appTaggingSecrets: Fr[]) {
    return await this.#getTaggingSecretsIndexes(
      appTaggingSecrets,
      this.#taggingSecretIndexesForRecipients
    );
  }
  ...
  #getTaggingSecretsIndexes(
    ...
    return this.db.transaction(() =>
      appTaggingSecrets.map(
        (secret) => storageMap.get(`${secret.toString()}`) ?? 0
      )
    );
  }
