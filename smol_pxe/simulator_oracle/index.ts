...
export class SimulatorOracle implements DBOracle {
  ...
  async getContractInstance(address: AztecAddress): Promise<ContractInstance> {
    ...
    if (!instance) {
      throw new Error(
        `No contract instance found for address ${address.toString()}`
      );
    }
    return instance;
  }
  ...
  async getFunctionArtifact(
    ...
    const artifact = await this.contractDataOracle.getFunctionArtifact(
      contractAddress,
      selector
    );
  ...
  async getFunctionArtifactByName(
    ...
    const instance = await this.contractDataOracle.getContractInstance(
      contractAddress
    );
    const artifact = await this.contractDataOracle.getContractArtifact(
  ...
  async getCommitmentIndex(commitment: Fr) {
    return await this.findLeafIndex(
      "latest",
  ...
  public async findLeafIndex(
    ...
    const [leafIndex] = await this.aztecNode.findLeavesIndexes(
      blockNumber,
      treeId,
      [leafValue]
    );
  ...
  public async getSiblingPath(
    ...
    switch (treeId) {
      case MerkleTreeId.NULLIFIER_TREE:
        return (
          await this.aztecNode.getNullifierSiblingPath(blockNumber, leafIndex)
        ).toFields();
      case MerkleTreeId.NOTE_HASH_TREE:
        return (
          await this.aztecNode.getNoteHashSiblingPath(blockNumber, leafIndex)
        ).toFields();
      case MerkleTreeId.PUBLIC_DATA_TREE:
        return (
          await this.aztecNode.getPublicDataSiblingPath(blockNumber, leafIndex)
        ).toFields();
      case MerkleTreeId.ARCHIVE:
        return (
          await this.aztecNode.getArchiveSiblingPath(blockNumber, leafIndex)
        ).toFields();
      default:
  ...
  public async getNullifierMembershipWitnessAtLatestBlock(nullifier: Fr) {
    return this.getNullifierMembershipWitness(
  ...
  public getLowNullifierMembershipWitness(
    ...
    return this.aztecNode.getLowNullifierMembershipWitness(
      blockNumber,
      nullifier
    );
  }
  ...
  public async getAppTaggingSecretAsSender(
    ...
    const secret = await this.#calculateTaggingSecret(
      contractAddress,
      sender,
      recipient
    );
  ...
  public async incrementAppTaggingSecretIndexAsSender(
    ...
    const contractName = await this.contractDataOracle.getDebugContractName(
      contractAddress
    );
    this.log.verbose(
  ...
  async #calculateTaggingSecret(
    contractAddress: AztecAddress,
    sender: AztecAddress,
    recipient: AztecAddress
  ) {
    const senderCompleteAddress = await this.getCompleteAddress(sender);
    const senderIvsk = await this.keyStore.getMasterIncomingViewingSecretKey(
      sender
    );
    const sharedSecret = computeTaggingSecret(
      senderCompleteAddress,
      senderIvsk,
      recipient
    );
    // Silo the secret to the app so it can't be used to track other app's notes
    const siloedSecret = poseidon2Hash([
      sharedSecret.x,
      sharedSecret.y,
      contractAddress,
    ]);
  ...
  async #getAppTaggingSecretsForContacts(
    ...
    const recipientIvsk = await this.keyStore.getMasterIncomingViewingSecretKey(
      recipient
    );
    ...
    const contacts = [
      ...this.db.getContactAddresses(),
      ...(await this.keyStore.getAccounts()),
    ].filter(
      (address, index, self) =>
        index === self.findIndex((otherAddress) => otherAddress.equals(address))
    );
    ...
    const indexes = await this.db.getTaggingSecretsIndexesAsRecipient(
      appTaggingSecrets
    );
    return appTaggingSecrets.map(
      (secret, i) => new IndexedTaggingSecret(secret, indexes[i])
    );
  }
  ...
  public async syncTaggedLogsAsSender(
    ...
    const appTaggingSecret = await this.#calculateTaggingSecret(
      contractAddress,
      sender,
      recipient
    );
    let [currentIndex] = await this.db.getTaggingSecretsIndexesAsSender([
      appTaggingSecret,
    ]);
    ...
    do {
      const currentTags = [...new Array(INDEX_OFFSET)].map((_, i) => {
        const indexedAppTaggingSecret = new IndexedTaggingSecret(
          appTaggingSecret,
          currentIndex + i
        );
      ...
      const indexOfFirstLog = possibleLogs.findIndex(
        (possibleLog) => possibleLog.length !== 0
      );
      currentEmptyFront =
        indexOfFirstLog === -1 ? INDEX_OFFSET : indexOfFirstLog;
    ...
    await this.db.setTaggingSecretsIndexesAsSender([
      new IndexedTaggingSecret(appTaggingSecret, newIndex),
    ]);
  ...
  public async syncTaggedLogs(
    ...
    for (const recipient of recipients) {
      ...
      const appTaggingSecrets = await this.#getAppTaggingSecretsForContacts(
        contractAddress,
        recipient
      );
        ...
        (acc, appTaggingSecret) => ({
          ...
          currentTagggingSecrets: acc.currentTagggingSecrets.concat([
            new IndexedTaggingSecret(
              appTaggingSecret.secret,
              Math.max(0, appTaggingSecret.index - INDEX_OFFSET)
            ),
          ]),
          ...
          maxIndexesToCheck: {
            ...
            ...{
              [appTaggingSecret.secret.toString()]:
                appTaggingSecret.index + INDEX_OFFSET,
            },
          },
      ...
      const { maxIndexesToCheck, secretsToIncrement, initialSecretIndexes } =
        searchState;
      ...
      while (currentTagggingSecrets.length > 0) {
        ...
        const currentTags = currentTagggingSecrets.map((taggingSecret) =>
          taggingSecret.computeTag(recipient)
        );
        ...
        logsByTags.forEach((logsByTag, logIndex) => {
          const { secret: currentSecret, index: currentIndex } =
            currentTagggingSecrets[logIndex];
          ...
          if (logsByTag.length > 0) {
            ...
            if (currentIndex >= initialSecretIndexes[currentSecretAsStr]) {
              ...
              maxIndexesToCheck[currentSecretAsStr] =
                currentIndex + INDEX_OFFSET;
            }
          }
          ...
          if (currentIndex < maxIndexesToCheck[currentSecretAsStr]) {
            const newTaggingSecret = new IndexedTaggingSecret(
              currentSecret,
              currentIndex + 1
            );
      ...
      result.set(
        ...
        logs.filter(
          (log, index, self) =>
            log.blockNumber <= maxBlockNumber &&
            index === self.findIndex((otherLog) => otherLog.equals(log))
        )
      );
    }
    return result;
  ...
  async #decryptTaggedLogs(
    ...
    const addressSecret = computeAddressSecret(
      recipientCompleteAddress.getPreaddress(),
      ivskM
    );
    ...
    for (const scopedLog of scopedLogs) {
      ...
      const outgoingNotePayload = L1NotePayload.decryptAsOutgoing(
        scopedLog.logData,
        ovskM,
        scopedLog.isFromPublic
      );
      ...
      if (incomingNotePayload || outgoingNotePayload) {
        if (
          incomingNotePayload &&
          outgoingNotePayload &&
          !incomingNotePayload.equals(outgoingNotePayload)
        ) {
          this.log.warn(
        ...
        const txEffect =
          txEffectsCache.get(scopedLog.txHash.toString()) ??
          (await this.aztecNode.getTxEffect(scopedLog.txHash));

        if (!txEffect) {
          this.log.warn(
            `No tx effect found for ${scopedLog.txHash} while decrypting tagged logs`
          );
          continue;
        }
  ...
  public async processTaggedLogs(
    ...
    if (incomingNotes.length || outgoingNotes.length) {
      ...
      incomingNotes.forEach((noteDao) => {
        this.log.verbose(
          `Added incoming note for contract ${
      ...
      outgoingNotes.forEach((noteDao) => {
        this.log.verbose(
          `Added outgoing note for contract ${noteDao.contractAddress} at slot ${noteDao.storageSlot}`
        );
      });
    }
    ...
    const nullifiersToCheck = currentNotesForRecipient.map(
      (note) => note.siloedNullifier
    );
    const currentBlockNumber = await this.getBlockNumber();
    const nullifierIndexes =
      await this.aztecNode.findNullifiersIndexesWithBlock(
        currentBlockNumber,
        nullifiersToCheck
      );
